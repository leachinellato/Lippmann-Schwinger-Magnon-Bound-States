# ═══════════════════════════════════════════════════════════════════════
#  ls_three_body.jl — Faddeev / Skornyakov–Ter-Martirosyan form of the
#  Lippmann–Schwinger equation for three hard-core bosons at K = 0.
# ═══════════════════════════════════════════════════════════════════════
...
"""
    E_pair_threshold(ms; pset = 1:ms.Ns)

Magnon-plus-pair threshold `min_p [ε(p) + E₂(−p)]`, from `det B₃(E;p) = 0`.  Returns
`nothing` if no pair is bound at any spectator momentum in `pset` (λ < λc), in
...
"""
function E_pair_threshold(ms::Mesh3; pset = 1:ms.Ns, tol = 1e-12, dmax = 50)
    best = nothing
    for p in pset
